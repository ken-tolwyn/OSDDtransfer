...
for required in \
  ...
  "${HOST_TRUNK}/registry-charts-native-dry-run" \
  "${HOST_TRUNK}/registry-images-native-count" \
  "${HOST_TRUNK}/registry-charts-native-count"; do
  if [[ ! -f "$required" ]]; then
...
grep -q '^1$' "${HOST_TRUNK}/registry-images-native-count"
grep -q '^1$' "${HOST_TRUNK}/registry-charts-native-count"

printf 'registry native test passed: %s %s\n' \
  "${HOST_TRUNK}/registry-images-native-count" \
  "${HOST_TRUNK}/registry-charts-native-count"
